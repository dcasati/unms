...
load_tf_enf() {
  export TF_CLUSTER_NAME=$(terraform output -json | jq .aks_cluster_name.value -r)
  export TF_RG_NAME=$(terraform output -json | jq .resource_group_name.value -r)
  export TF_JUMPBOX=$(terraform output -json | jq .jumpbox.value.ssh -r)
}
